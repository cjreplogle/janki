...
def import_jank(path: str, parent=None) -> None:
    ...
    def _rest(deck_res):
        ...
        if found["qb"]:
            ...
            lines.append("Question banks: %d installed/updated" % ok)
        # Lecture → tag maps
        if found["json"]:
            ok = 0
            for p in found["json"]:
        ...
        if man.get("notes"):
            msg += "\n\n" + str(man["notes"])[:1500]
        if found["qb"]:
            msg += ("\n\nTip: Settings → Practice → Question Bank → “Load Question Banks to "
                    "Anki” builds the Practice decks from the new banks.")
        if found["rp"]:
...
def import_jank_dialog(parent=None) -> None:
...
def _local_sources() -> dict:
    ...
    try:
        from . import reword
        out["rp"] = sum(1 for r in reword._load().values()
...
def _pack_bank(bank_dir: str, dest_dir: str, stem: str) -> str:
    """Re-pack an installed bank folder as a .qb (zip) for the bundle."""
    import zipfile
    ...
    with zipfile.ZipFile(out, "w", zipfile.ZIP_DEFLATED) as z:
        for base, _dirs, files in os.walk(bank_dir):
            for f in files:
                ...
                z.write(full, os.path.relpath(full, bank_dir))
    return out


def _export_rp(dest_dir: str) -> str:
    """The rephrasing store as one .rp (plain-text rephrasings; list-reorder HTML variants are
    left out — they're rebuilt locally)."""
    from . import reword
    items = [{"id": k, "variants": r["variants"]} for k, r in sorted(reword._load().items())
             if isinstance(r, dict) and r.get("variants") and not r.get("html")]
    out = os.path.join(dest_dir, "rephrasings.rp")
    with open(out, "w", encoding="utf-8") as f:
        json.dump({"type": "janki-rephrase", "version": 1, "items": items}, f,
...
def packager_dialog(parent=None) -> None:
    ...
    for b in src["banks"]:
        _leaf(g_banks, b["name"], "%d questions" % b["count"] if b["count"] else "",
              ("bank", b))
    if not src["banks"]:
    ...
    if src["rp"]:
        _leaf(g_rp, "All stored rephrasings", "%d card sides" % src["rp"], ("rp", None))
    else:
